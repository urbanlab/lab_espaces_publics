<header class="banner sticky top-0 border-b border-black bg-white z-20">
  <div class="flex max-sm:flex-wrap justify-between sm:h-20 p-4 md:py-0  md:items-center">
    ...
    {{-- @if (has_nav_menu('primary_navigation'))
      <nav class="sticky nav-primary hidden w-screen md:w-3/4 max-sm:pt-16 max-sm:h-screen max-sm:order-last md:flex text-lg font-bold" id="navbar-hamburger" aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
        {!! wp_nav_menu(['theme_location' => 'primary_navigation', 'menu_class' => 'menu', 'container_aria_label' => 'Main']) !!}
      </nav>
    @endif --}}
    <x-navigation />
